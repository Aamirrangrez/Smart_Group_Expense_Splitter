...
export default function DashboardPage() {
  ...
  if (loading) return <div className="page-loader"><div className="spinner"></div> Loading dashboard...</div>;
    ...
    <div className="page-container">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Hey {user?.name?.split(' ')[0]} 👋 Here's your financial overview</p>
        </div>
      </div>
                ...
                return (
                  <div key={exp.id} className="expense-item" onClick={() => navigate(`/groups/${exp.group.id}`)}>
                    ...
                    <div className="expense-info">
                      ...
                      <div className="expense-meta">{exp.group.name} · {timeAgo(exp.createdAt)} · paid by {exp.paidBy.id === user?.id ? 'you' : exp.paidBy.name}</div>
                    </div>
                    <div className="expense-amount-col">
                      <div className="expense-amount">{formatCurrency(exp.amount, exp.group.currency)}</div>
                      {myShare && <div className="expense-split-info">your share: {formatCurrency(myShare.amount, exp.group.currency)}</div>}
                    </div>
                  </div>
                );
              })}
